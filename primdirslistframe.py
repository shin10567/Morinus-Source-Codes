...
class PrimDirsListFrame(wx.Frame):
	...
	def __init__(self, parent, chrt, options, pds, title):
		wx.Frame.__init__(self, parent, -1, title, wx.DefaultPosition, size=wx.Size(1100, 700))
		...
		self.ID_Back = 20
		self.tb.AddTool(20, "Back", back_bmp, shortHelp=mtexts.txts["Back"])
		self.Bind(wx.EVT_TOOL, self.OnBack, id=self.ID_Back)

		self.ID_Forward = 30
		self.tb.AddTool(30, "Forward", forward_bmp, shortHelp=mtexts.txts["Forward"])
		self.Bind(wx.EVT_TOOL, self.OnForward, id=self.ID_Forward)

		self.ID_End = 40
		self.tb.AddTool(40, "End", toend_bmp, shortHelp=mtexts.txts["End"])
