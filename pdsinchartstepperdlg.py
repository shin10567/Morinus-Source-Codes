...
class PDsInChartStepperDlg(wx.Dialog):
	...
	def __init__(self, parent, chrt, y, m, d, t, direct, arc, opts, terrestrial):
		...
		# method.
		# Phoenix(wxPython 4.x): 일반 생성자로 생성
		# Phoenix-safe 2-phase create
		# 1) precreate wrapper
...
#		vsizer.Add(self.dateckb, 0, wx.ALIGN_LEFT|wx.LEFT|wx.RIGHT|wx.TOP, 5)
		...
		fgsizer = wx.FlexGridSizer(rows=2, cols=3, vgap=8, hgap=8)
		...
		self.da = wx.TextCtrl(self, -1, '', validator=floatvalidator.FloatValidator(0.0, None), size=(100,-1))
			...
			if util.checkDate(int(self.year.GetValue()), int(self.month.GetValue()), int(self.day.GetValue())):
				...
				if not self.terrestrial:
					...
					if self.options.pdinchartterrsecmotion:
						...
						pdchart.fortune.calcFullAstronomicalProc(self.chart.fortune, da, self.chart.obl[0])
					...
					pdchart.fortune.recalcForMundaneChart(pdchart.fortune.fortune[fortune.Fortune.LON], pdchart.fortune.fortune[fortune.Fortune.LAT], pdchart.fortune.fortune[fortune.Fortune.RA], pdchart.fortune.fortune[fortune.Fortune.DECL], pdchart.houses.ascmc2, pdchart.raequasc, pdchart.obl[0], pdchart.place.lat)
