	...
	def __init__(self, chrt, options, pdrange, direction, abort):
		...
		try:
			...
			if opts.ayanamsha != 0:
				...
				self.chart.ayanamsha = astrology.swe_get_ayanamsa_ut(self.chart.time.jd)
				pflag |= astrology.SEFLG_SIDEREAL
			if getattr(opts, 'topocentric', False):
				pflag |= astrology.SEFLG_TOPOCTR

			# planets는 시그니처를 정확히 맞춰 새로 생성
			self.chart.planets = planets.Planets(self.chart.time.jd, opts.meannode,
												pflag, self.chart.place.lat,
												self.chart.houses.ascmc2,
												self.chart.raequasc,
												getattr(self.chart, 'nolat', False),
												self.chart.obl[0])

			# 2) 미드포인트/평행점 등도 현재 planets 기준으로 다시 만들기
			if hasattr(self.chart, 'midpoints'):
				if hasattr(self.chart.midpoints, 'recalc'):
					self.chart.midpoints.recalc(self.chart.planets, self.chart.obl[0])
				else:
					import midpoints
					self.chart.midpoints = midpoints.MidPoints(self.chart.planets, self.chart.obl[0])

			if hasattr(self.chart, 'zodpars'):
				if hasattr(self.chart.zodpars, 'recalc'):
					self.chart.zodpars.recalc(self.chart.planets, self.chart.obl[0])
				else:
					import zodpars
					self.chart.zodpars = zodpars.ZodPars(self.chart.planets, self.chart.obl[0], self.options)
